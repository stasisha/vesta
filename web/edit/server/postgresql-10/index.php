...
if (!empty($_POST['save'])) {
    ...
    if ((empty($_SESSION['error_msg'])) && (!empty($_POST['v_config']))) {
        ...
        fwrite($fp, str_replace("\r\n", "\n",  $_POST['v_config']));
        fclose($new_conf);
        exec (VESTA_CMD."v-change-sys-service-config ".$new_conf." postgresql-10 " .$v_restart, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        unlink($new_conf);
    }
...
// List config
exec (VESTA_CMD."v-list-sys-pgsql10-config json", $output, $return_var);
